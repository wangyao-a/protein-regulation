...
def jiao(dxc,dxcp,dxcpx,F1,F2,F3,F4,monum,monump,monumpx):
    ...
    for x2 in range(pair):
        F0+=Kp2*(l2l[x2]-Xpole)
        F7+=Kp2*(l3r[x2]-Xpo)
        F5+=Kp4*(Xkin1-Xkin-kdis)
        F6+=Kp4*(Xkin-Xkin1+kdis)
        if abs(F1[0])>0.00001 or abs(F4[0])>0.00001 or abs(F3[x2])>0.00001 or abs(F2[x2])>0.00001:
            Fcc=1
    ...
    a00=round((-Kp1-2*Kp2),3);a10=Kp2;a30=Kp1;a80=Kp2;a100=Kp1
    ...
    for cx0 in range(len(ji[0])):
        ...
        for cx1 in range(len(ji[1])):
            ...
            b=np.array([round(-F0,14),round(-F7,14),round(-F5,14),round(-F6,14),round(b4,14),round(b5,14),round(b6,14),round(b7,14),round(b9,14),round(b10,14)])
            try:
                
                A=np.array([[a00,a10,0,a30,0,0,0,0,a80,0],[0,0,a21,0,a41,0,0,a71,0,a91],
                             [0,0,0,a32,0,a52,a62,0,0,0],[0,0,0,0,a43,a53,a63,0,0,0],
                             [a04,a0104,a24,a34,0,a54,0,0,0,0],[a05,a15,a25,a35,a45,0,0,0,a85,0],
                             [0,a16,a26,a36,a46,0,0,a76,0,a96],[0,a17,a27,0,a47,0,a67,a77,0,0],
                             [a09,a019,0,0,0,0,0,0,a19,a29],
                             [0,0,a0210,0,0,0,0,a710,a110,a210]])
            ...
            de=[mtui[0],[mtui[1],mtui[8]],[mtui[2],mtui[9]],[mtui[3],0],[mtui[4],0],mtui[5],mtui[6],mtui[7]]
            #print(de,'xx')
            #print(b)
            chezq=0
                    ...
                    if xx3==1 and de[4][x3]-de[1][x3]>=0:
                        ...
                        for ton1 in range(monumpx[x3]):
                            ...
                            if abs(jisuanpx[x3][ton1])<=xdmotor and jisuanpx[x3][ton1]+de[4][x3]-de[1][x3]>xdmotor:
                                checkpxk+=1
                    if xx3==0 and de[4][x3]-de[1][x3]<=0:#dxcp
                        checkpx0=0#################
                        checkpxk=0################
                        for ton1 in range(monumpx[x3]):
                            if abs(jisuanpx[x3][ton1])<=xdmotor and jisuanpx[x3][ton1]+de[4][x3]-de[1][x3]<-xdmotor:
    ...
    if sum(zq)!=pair:
        for xqq in range(pair):
            if abs(F3[xqq])>0.001 or abs(F2[xqq])>0.001 :
                dpn=xqq
                break
        ...
        for xn in range(num1[dpn]+numpx2[dpn]+num2[dpn]+numpx1[dpn]+1):#MT2
            ...
            for xn1 in range(nump2[dpn]+nump1[dpn]+1):#MT1
                ...
                if abs(tui2-lastui2)<0.0001 or abs(tui1)+abs(tui2)==0:
                    continue
                lastui2=tui2#####MT1往前tui1,MT2往前tui2
                F0=-FPE+Kp1*(l1l[0]-Xpole)
                F7=FPE+Kp1*(l4r[0]-Xpo)
                F5=FPE+Kp3*(l1r[0]-Xkin)
                F6=-FPE+Kp3*(l4l[0]-Xkin1)
                ...
                Fcc=0
                for x2 in range(pair):
                    if abs(F1[0])+abs(F2[x2])+abs(F3[x2])+abs(F4[0])>0.01:
                        Fcc=1
                        break
            ...
            for xn1 in range(numpx2[dpn]+numpx1[dpn]+1):#MT4
                ...
                for x2 in range(pair):
                    ...
                    if x2!=dpn:
                        ...
                        for ton1 in range(monumpx[x2]):
                            jisuanpx1[x2].append(jisuanpx[x2][ton1])
                #print(jisuan1,jisuanp1,jisuanpx1,F0,F1,F2,F3,F4,F5,F6,F7,dpn,'cc2')
                tuf,de[0],de[1],de[2],de[3],de[4],de[5],de[6],de[7]=jiaocheck(jisuan1,jisuanp1,jisuanpx1,F0,F1,F2,F3,F4,F5,F6,F7)
                if tuf==1:
                    de[2][dpn]+=tui1
                    if dpn==0:
                                ...
                                for xc0 in xa3:
                                    ...
                                    for xc0p in xa4:
                                        ...
                                        for xc0px in xa5:
                                            ...
                                            ji[x2].append([cnn0,cnnk,cnn0p,cnnkp,cnn0px,cnnkpx,xx1,xx2,xx3,ta1,ta2,ta3,ta4,ta5,ta6])
    a00=round((-Kp1-2*Kp2),3);a10=Kp2;a30=Kp1;a80=Kp2;a100=Kp1
    a21=Kp2;a41=Kp1;a71=round((-Kp1-2*Kp2),3);a91=Kp2;a111a=Kp1
    a32=Kp3;a52=round((-Kp3-2*Kp4),3);a62=round(Kp4*2,3);a102=Kp3
    a43=Kp3;a53=round(Kp4*2,3);a63=round((-Kp3-2*Kp4 ),3);a113=Kp3
    zq=[0 for i in range(pair)]
    cxx=[0 for i in range(pair)]
    for cx0 in range(len(ji[0])):
        cxx[0]=cx0
...
def energy(x1,i):
    ...
    for x2 in range(pair):
        if x2==0:
            ypsl11+=Kp3*(((Xkin+zj5[x1][i]-l1r[x2]-zj3[x1][i][x2])**2+(Xkin1+zj6[x1][i]-l4l[x2]-zj4[x1][i][x2])**2)/2)+Kp1*((Xpole+zj0[x1][i]-l1l[x2]-zj3[x1][i][x2])**2+(Xpo+zj7[x1][i]-l4r[x2]-zj4[x1][i][x2])**2)/2
            ypsl22+=Kp3*(((Xkin+js5[x1][i]-l1r[x2]-js3[x1][i][x2])**2+(Xkin1+js6[x1][i]-l4l[x2]-js4[x1][i][x2])**2)/2)+Kp1*((Xpole+js0[x1][i]-l1l[x2]-js3[x1][i][x2])**2+(Xpo+js7[x1][i]-l4r[x2]-js4[x1][i][x2])**2)/2
        
        ypsl11+=Kp2*((Xpole+zj0[x1][i]-l2l[x2]-zj1[x1][i][x2])**2+(Xpo+zj7[x1][i]-l3r[x2]-zj2[x1][i][x2])**2)/2+Kp4*((Xkin1+zj6[x1][i]-Xkin-zj5[x1][i]-kdis)**2)/2
        ypsl22+=Kp2*((Xpole+js0[x1][i]-l2l[x2]-js1[x1][i][x2])**2+(Xpo+js7[x1][i]-l3r[x2]-js2[x1][i][x2])**2)/2+Kp4*((Xkin1+js6[x1][i]-Xkin-js5[x1][i]-kdis)**2)/2
...
def energyp(x1,i):
    ...
    for x2 in range(pair):
        if x2==0:
            ypsl11+=Kp3*(((Xkin+zj5p[x1][i]-l1r[x2]-zj3p[x1][i][x2])**2+(Xkin1+zj6p[x1][i]-l4l[x2]-zj4p[x1][i][x2])**2)/2)+Kp1*((Xpole+zj0p[x1][i]-l1l[x2]-zj3p[x1][i][x2])**2+(Xpo+zj7p[x1][i]-l4r[x2]-zj4p[x1][i][x2])**2)/2
            ypsl22+=Kp3*(((Xkin+js5p[x1][i]-l1r[x2]-js3p[x1][i][x2])**2+(Xkin1+js6p[x1][i]-l4l[x2]-js4p[x1][i][x2])**2)/2)+Kp1*((Xpole+js0p[x1][i]-l1l[x2]-js3p[x1][i][x2])**2+(Xpo+js7p[x1][i]-l4r[x2]-js4p[x1][i][x2])**2)/2
        
        ypsl11+=Kp2*((Xpole+zj0p[x1][i]-l2l[x2]-zj1p[x1][i][x2])**2+(Xpo+zj7p[x1][i]-l3r[x2]-zj2p[x1][i][x2])**2)/2+Kp4*((Xkin1+zj6p[x1][i]-Xkin-zj5p[x1][i]-kdis)**2)/2
        ypsl22+=Kp2*((Xpole+js0p[x1][i]-l2l[x2]-js1p[x1][i][x2])**2+(Xpo+js7p[x1][i]-l3r[x2]-js2p[x1][i][x2])**2)/2+Kp4*((Xkin1+js6p[x1][i]-Xkin-js5p[x1][i]-kdis)**2)/2
...
def energypx(x1,i):
    ...
    for x2 in range(pair):
        if x2==0:
            ypsl11+=Kp3*(((Xkin+zj5px[x1][i]-l1r[x2]-zj3px[x1][i][x2])**2+(Xkin1+zj6px[x1][i]-l4l[x2]-zj4px[x1][i][x2])**2)/2)+Kp1*((Xpole+zj0px[x1][i]-l1l[x2]-zj3px[x1][i][x2])**2+(Xpo+zj7px[x1][i]-l4r[x2]-zj4px[x1][i][x2])**2)/2
            ypsl22+=Kp3*(((Xkin+js5px[x1][i]-l1r[x2]-js3px[x1][i][x2])**2+(Xkin1+js6px[x1][i]-l4l[x2]-js4px[x1][i][x2])**2)/2)+Kp1*((Xpole+js0px[x1][i]-l1l[x2]-js3px[x1][i][x2])**2+(Xpo+js7px[x1][i]-l4r[x2]-js4px[x1][i][x2])**2)/2
        
        ypsl11+=Kp2*((Xpole+zj0px[x1][i]-l2l[x2]-zj1px[x1][i][x2])**2+(Xpo+zj7px[x1][i]-l3r[x2]-zj2px[x1][i][x2])**2)/2+Kp4*((Xkin1+zj6px[x1][i]-Xkin-zj5px[x1][i]-kdis)**2)/2
        ypsl22+=Kp2*((Xpole+js0px[x1][i]-l2l[x2]-js1px[x1][i][x2])**2+(Xpo+js7px[x1][i]-l3r[x2]-js2px[x1][i][x2])**2)/2+Kp4*((Xkin1+js6px[x1][i]-Xkin-js5px[x1][i]-kdis)**2)/2
        for xn in range(nu[x2]):
            if kina[x2][xn]==1 and kinb[x2][xn]==1:
...
def energy0():
    ...
    for x2 in range(pair):
        if x2==0:
            ypsl0+=Kp3*(((Xkin-l1r[x2])**2+(Xkin1-l4l[x2])**2)/2)+Kp1*((Xpole-l1l[x2])**2+(Xpo-l4r[x2])**2)/2
        ypsl0+=Kp2*((Xpole-l2l[x2])**2+(Xpo-l3r[x2])**2)/2+Kp4*((Xkin1-Xkin-kdis)**2)/2
        for xn in range(nu[x2]):
...
def checkF():
    ...
    for x2 in range(pair):
        if abs(F3[x2])+abs(F2[x2])>0.0001:
            Fcc=1
    if abs(F1[0])+abs(F4[0])+abs(F0)+abs(F7)+abs(F5)+abs(F6)>0.0001 or Fcc==1:
        #print(F1,F2,F3,F4,F0,F7,F5,F6,a)
        #print(qq)
...
for x1 in range(pair):
    ...
    for xn in range(round(Number*random.uniform(nant*2,nant*2+1))):
        ...
        kinb[x1].append(round(random.uniform(0,0.9)))
        ypsl1[x1].append(0)
        dxc[x1].append(0)
        pan[x1].append(0)
        ypsl2[x1].append(0)
    ...
    if x1==0: 
        for xn in range(round(Number*random.uniform(nant,nant+1))):
            ...
            kinap[x1].append(round(random.uniform(0,0.8)))
            kinbp[x1].append(round(random.uniform(0.2,1)))
            dxcp[x1].append(0)
            panp[x1].append(0)
            ypsl1p[x1].append(0)
            ypsl2p[x1].append(0)
...
##################################################################
        ...
        for xbb in range(nu1[x1]):
            ...
            if kinap[x1][xbb]==0 and kinbp[x1][xbb]==1:
                ...
                if kinbp[x1][xbb]==1 and kinap[x1][xbb]==1:
                    lian1=0
                    for xn in range(nu1[x1]):
                        if kinap[x1][xn]==1 and xn!=xbb and abs(l1r[x1]-round((l1r[x1]-motorp[x1][xbb][1])/d)*d-motorp[x1][xn][0])<1:
                            lian1=1
                            break
                    if lian1==1:
                        if l1r[x1]+round((motorp[x1][xbb][1]-l1r[x1])/d)*d>=motorp[x1][xbb][1]:
                            la=(round((motorp[x1][xbb][1]-l1r[x1])/d)-1)*d
                        if l1l[x1]+round((motorp[x1][xbb][1]-l1r[x1])/d)*d<motorp[x1][xbb][1]:
                            la=(round((motorp[x1][xbb][1]-l1r[x1])/d)+1)*d
                        for xn in range(nu1[x1]):
                            if kinap[x1][xn]==1 and xn!=xbb and abs(l1r[x1]+la-motorp[x1][xn][0])<1:
                                lian1=2
                                break
                    if lian1==0:
                        motorp[x1][xbb][0]=l1r[x1]+round((motorp[x1][xbb][1]-l1r[x1])/d)*d
                    if lian1==1:
                        motorp[x1][xbb][0]=l1r[x1]+la
                    if lian1==2:
                        kinap[x1][xbb]=0
                    pan1=1
                    monump[x1]+=1
    ...
    if a%5000==0:
        ...
        dataframe.to_csv('./2enuckmt1'+str(koffn)+str(Foffn)+str(FPE)+str(pi1)+str(alp)+str(Fxing)+str(MTnumber)+str(B)+str(Fp0)+str(Kp1)+str(Kp2)+str(Kp3)+str(vd)+str(kon)+str(vp1)+'ss11.csv',index=False,sep=',')
...
ypsl0px,ypsl1p[x1][xbb],ypsl2p[x1][xbb]
...
numa1[sik][sn][nn][0][xbb][1]


# In[36]:


ckF,de0,de1,de2,de3,de4,de5,de6,de7=checkF()#[3.1807889655510735e-14, 0.4100000000004904] [6.811218256075335e-14, 3.4583447217073626e-14] [-7.03603841856193e-14, 5.24469356832924e-13] [4.593547764386585e-14, -9.325873406851315e-14] 0.41000000000030923 -9.103828801926284e-14 -4.440892098500626e-15 4.440892098500626e-15#checkF()
if ckF==1:
    for x2 in range(pair):
        for xn in range(nu[x2]):
            if kina[x2][xn]==1:
...
time1,MTX1,MTX2,MTzn6,MTzn7,MTzn8,MTzn18,MTzn9,MTzn11,MTzn12,MTzn13,MTzn14,MTzn15,MTzn16,MTk1,MTi1,MTzn3,MTzn1,MTzn2,MTzn17,MTzn20,MTzn21,MTzn10,MTzn19,MTzn22,MTzn23
...
desia2,deska1,posa1,posa4,posa2,ximta2,xkmta1,ximta3,xkmta4,a*h
...
jisuan1,jisuanp1,jisuanpx1,F0,F1,F2,F3,F4,F5,F6,F7,dpn,'cc2'
            ...
            for sik in range(2):
                ...
                for ton1 in range(Nma1[sik][ssx][xn][1]):
                    if kinam1[sik][ssx][xn][1][ton1]==1 and kinbm1[sik][ssx][xn][1][ton1]==1:
                        ...
                        for to2 in range(xn,MTnumber-1):
                            dxnuma1+=-overlap[sik][ssx][3][to2]+overlap[sik][ssx][2][to2]
                        print(dxnuma1,numa1[sik][ssx][xn][1][ton1][0],numa1[sik][ssx][xn][1][ton1][1],node1[sik][ssx][3][nn+1],node1[sik][ssx][3][nn],node1[sik][ssx][2][nn],node1[sik][ssx][2][nn+1],'1')
                        F4[sik]-=Kpn5*dxnuma1
                        F3[sik]+=Kpn5*dxnuma1
...
zj0[x1][i],zj1[x1][i],zj2[x1][i],zj3[x1][i],zj4[x1][i],zj5[x1][i],zj6[x1][i],zj7[x1][i]
